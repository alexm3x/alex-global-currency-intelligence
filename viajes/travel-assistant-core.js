(() => {
  ...
  function deriveFromComments(comments) {
    ...
    const preferenceSignals = {
      gastronom: 'gastronomía', golf: 'golf', buce: 'buceo', playa: 'playa y descanso', esqu: 'esquí',
      museo: 'cultura e historia', cultur: 'cultura e historia', spa: 'bienestar y spa', compr: 'compras', negocio: 'negocios y conectividad'
    };
    Object.entries(preferenceSignals).forEach(([needle, label]) => { if (lower.includes(needle)) preferences.push(label); });
    ...
    constraintSignals.forEach(([pattern, label]) => { if (pattern.test(clean)) constraints.push(label); });
  ...
  function createProfile(raw = {}) {
    ...
    const childAges = (Array.isArray(raw.childAges) ? raw.childAges : []).slice(0, childCount).map(age => integer(age, 0, 17, 8));
    ...
    const hardConstraints = list([...(raw.hardConstraints || []), ...derived.constraints, ...(concerns.includes('accessibility') ? ['Accesibilidad verificable'] : []), ...(raw.directPreference === 'required' ? ['Vuelo directo obligatorio'] : [])]);
    const destinationMode = ['fixed', 'region', 'open'].includes(raw.destinationMode) ? raw.destinationMode : text(raw.destination, 120) ? 'fixed' : 'open';
    const destinationValues = destinationMode === 'open' ? [] : list([raw.destination, ...(raw.destinationValues || [])], 8);
    ...
    const normalizedTotal = basis === 'person' ? amount * (adults + childCount) : basis === 'night' ? amount * rooms * nights : amount;
    const origin = text(raw.originCity || raw.origin, 120);

    return {
      schema_version: VERSION,
      trip_id: text(raw.tripId, 80) || tripId(),
      created_at: raw.createdAt || new Date().toISOString(),
      origin: { city: origin, airports: list(raw.originAirports?.length ? raw.originAirports : origin ? [origin] : [], 5) },
      destination_scope: { mode: destinationMode, values: destinationValues },
      dates: { start, end, month: text(raw.month, 7) || null, flex_days: integer(raw.flexDays, 0, 31, 0), nights_min: integer(raw.nightsMin, 1, 60, nights), nights_max: integer(raw.nightsMax, 1, 90, nights) },
      travelers: { adults, children: childAges, rooms, relation: text(raw.groupType, 40) || 'solo', room_preferences: list(raw.roomPreferences), accessibility: text(raw.accessibility, 500) },
      budget: { amount, normalized_total: normalizedTotal, currency: /^[A-Z]{3}$/.test(raw.currency || '') ? raw.currency : 'MXN', basis, includes: list(raw.budgetIncludes?.length ? raw.budgetIncludes : ['flights','lodging','destination']), strictness: ['strict','moderate','opportunity'].includes(raw.strictness) ? raw.strictness : 'moderate', contingency_pct: integer(raw.contingencyPct, 0, 50, 10) },
      transport: { cabin: ['economy','premium','business','first'].includes(raw.cabin) ? raw.cabin : 'economy', direct_preference: ['required','preferred','indifferent'].includes(raw.directPreference) ? raw.directPreference : 'preferred', max_stops: integer(raw.maxStops, 0, 4, raw.directPreference === 'required' ? 0 : 1), max_total_hours: raw.maxTotalHours ? clamp(raw.maxTotalHours, 1, 72, null) : null },
      lodging: { types: list(raw.lodgingTypes?.length ? raw.lodgingTypes : ['hotel']), category_min: raw.categoryMin ? clamp(raw.categoryMin, 1, 5, null) : null, location_preferences: list(raw.locationPreferences) },
  ...
  function analyzeProfile(profile) {
    ...
    const hasBudget = Number(profile.budget.normalized_total) > 0;
    const budgetPerTraveler = hasBudget ? profile.budget.normalized_total / Math.max(1, travelers) : null;
    const tensions = [];
    if (hasBudget && business && budgetPerTraveler < 90000) tensions.push('presupuesto frente a cabina premium');
    ...
    if (hasBudget && profile.dates.flex_days === 0 && profile.budget.strictness === 'strict') tensions.push('fechas fijas frente a presupuesto estricto');
    ...
    if (validation.valid && tensions.length) viability = hasBudget && business && budgetPerTraveler < 45000 ? 'low' : 'medium';
    ...
    return { viability, tension: tensions[0] || 'No se detecta una tensión material inicial.', strategy, travelers, nights: nightsBetween(profile.dates.start, profile.dates.end, profile.dates.nights_min), validation };
  }
  ...
  function destinationMatches(destination, profile) {
    ...
    const haystack = [destination.city, destination.country, destination.airport, ...(destination.tags || [])].join(' ').toLowerCase();
