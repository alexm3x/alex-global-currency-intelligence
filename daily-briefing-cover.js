(() => {
  ...
  async function loadReport(){
    try{
      ...
      if(data.schemaVersion!==2||!data.date||!data.title||!data.executiveSummary||!Array.isArray(data.audio?.chapters)||data.audio.chapters.length<7) throw new Error('Contrato editorial inválido');
      REPORT=data;
    }catch(err){
