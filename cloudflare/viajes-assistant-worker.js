...
const researchSchema = {
  ...
  properties: {
    ...
    sources: {
      ...
      items: {
        type: 'object', additionalProperties: false,
        properties: { title: { type: 'string', maxLength: 180 }, url: { type: 'string', maxLength: 600 }, type: sourceTypeSchema },
...
const premiumEvidenceSchema = {
  type: 'object', additionalProperties: false,
  properties: {
    value: { type: ['number','null'], minimum: -100, maximum: 500 },
    basis: { type: 'string', maxLength: 260 },
    source_url: { type: 'string', maxLength: 600 }
  },
    ...
    windows: {
      ...
      items: {
        ...
        properties: {
          ...
          flight_observed: observedPriceSchema,
          lodging_observed: observedPriceSchema,
          weather: evidenceSignalSchema,
          saturation: evidenceSignalSchema,
          logistics: evidenceSignalSchema,
          event_premium: premiumEvidenceSchema,
          cautions: { type: 'array', maxItems: 6, items: { type: 'string', maxLength: 220 } }
        },
        required: ['id','start','end','summary','flight_observed','lodging_observed','weather','saturation','logistics','event_premium','cautions']
      }
    },
    sources: {
...
async function readJsonBounded(request) {
  ...
  while (true) {
    ...
    if (received > MAX_BODY_BYTES) { await reader.cancel('payload_too_large'); throw new Error('payload_too_large'); }
    chunks.push(value);
  }
...
function cleanUrl(value) {
  try { const url = new URL(String(value || '')); if (url.protocol !== 'https:') return ''; url.hash = ''; return url.href.slice(0, 600); }
  catch { return ''; }
}
...
function safeWindows(candidate) {
  if (!Array.isArray(candidate) || !candidate.length || candidate.length > 8) throw new Error('invalid_windows');
  const out=[];
  for (const raw of candidate) {
    const id=cleanText(raw?.id,100), start=cleanText(raw?.start,10), end=cleanText(raw?.end,10);
    if (!id || !/^\d{4}-\d{2}-\d{2}$/.test(start) || !/^\d{4}-\d{2}-\d{2}$/.test(end) || Date.parse(end) < Date.parse(start)) throw new Error('invalid_windows');
    const days=Math.round((Date.parse(end)-Date.parse(start))/86400000)+1;
    if (days < 2 || days > 30) throw new Error('invalid_windows');
    out.push({ id, start, end, matched_item_ids:(Array.isArray(raw.matched_item_ids)?raw.matched_item_ids:[]).slice(0,14).map(value=>cleanText(value,100)), preliminary_score:Number.isFinite(Number(raw.preliminary_score))?Number(raw.preliminary_score):null });
  }
  return out;
}
...
async function openAI(body, env, timeout = 36000) {
  const response = await fetch('https://api.openai.com/v1/responses', {
    method: 'POST', headers: { authorization: `Bearer ${env.OPENAI_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify(body), signal: AbortSignal.timeout(timeout)
  });
  if (!response.ok) { const detail = cleanText(await response.text(), 300); throw new Error(`openai_${response.status}${detail ? `_${detail}` : ''}`); }
...
async function summarize(profile, env) {
  const { payload, requestId } = await openAI({
    model: env.OPENAI_MODEL || 'gpt-5', max_output_tokens: 420,
    input: [
...
function researchPrompt(profile) {
  ...
  const temporal = mode === 'known_dates' ? `${profile.dates?.start || ''} through ${profile.dates?.end || ''}` : `${profile.planning?.period_approx || profile.dates?.month || 'period not specified'}; duration ${profile.planning?.duration_days || profile.dates?.nights_min || 4} days`;
...
function sanitizeSources(candidate) {
  const sources=[], sourceMap=new Map();
  for (const source of Array.isArray(candidate) ? candidate : []) {
    const url=cleanUrl(source.url);if(!url||sourceMap.has(url))continue;
    const clean={title:cleanText(source.title,180),url,type:cleanText(source.type,30)||'other'};sourceMap.set(url,clean);sources.push(clean);
  }
  return {sources,sourceMap};
}

function sanitizeResearch(parsed, profile) {
  const {sources,sourceMap}=sanitizeSources(parsed.sources);
  const items=[];
  for (const raw of Array.isArray(parsed.items) ? parsed.items : []) {
    const url=cleanUrl(raw.source_url), listed=Boolean(url&&sourceMap.has(url));
    const status=raw.verification_status==='confirmed'&&listed?'confirmed':raw.verification_status==='estimated'?'estimated':'pending';
    items.push({ ...raw, id:cleanText(raw.id,100), name:cleanText(raw.name,180), venue:cleanText(raw.venue,160), location:cleanText(raw.location,200), source_title:cleanText(raw.source_title,180), source_url:listed?url:'', verification_status:status, why_relevant:cleanText(raw.why_relevant,420), price_observed:{amount:Number.isFinite(Number(raw.price_observed?.amount))?Number(raw.price_observed.amount):null,currency:cleanText(raw.price_observed?.currency,8),note:cleanText(raw.price_observed?.note,160),observed_at:cleanText(raw.price_observed?.observed_at,40)}, event_premium_pct:Number.isFinite(Number(raw.event_premium_pct))?Number(raw.event_premium_pct):null });
  }
  return { contract:CONTRACT_VERSION,destination:cleanText(parsed.destination||profile.destination_scope.values.join(', '),160),mode:profile.planning?.mode==='inverse_dates'?'inverse_dates':'known_dates',verified_at:new Date().toISOString(),summary:cleanText(parsed.summary,700),items,sources,cautions:(Array.isArray(parsed.cautions)?parsed.cautions:[]).map(value=>cleanText(value,260)).filter(Boolean).slice(0,10) };
}

async function research(profile, env) {
  const { payload, requestId } = await openAI({
    model: env.OPENAI_RESEARCH_MODEL || env.OPENAI_MODEL || 'gpt-5', tools: [{ type: 'web_search', search_context_size: 'high' }], max_output_tokens: 6000,
    input: [
...
function windowResearchPrompt(profile, windows) {
  const destination=profile.destination_scope.values.join(', '), origin=profile.origin?.city||profile.origin?.airports?.[0]||'not specified', travelers=(profile.travelers?.adults||1)+(profile.travelers?.children?.length||0), rooms=profile.travelers?.rooms||1;
  return `Compare only these candidate travel windows for ${destination}: ${JSON.stringify(windows)}. Origin: ${origin}. Travelers: ${travelers}. Rooms: ${rooms}. Cabin: ${profile.transport?.cabin||'economy'}. Priorities: ${(profile.priorities||[]).join(', ')||'open'}.
...
function sanitizeObservation(raw, sourceMap) {
  const url=cleanUrl(raw?.source_url), verified=Boolean(url&&sourceMap.has(url));
  return { amount:verified&&Number.isFinite(Number(raw?.amount))?Number(raw.amount):null, currency:cleanText(raw?.currency,8), note:cleanText(raw?.note,220), observed_at:cleanText(raw?.observed_at,40), source_title:verified?cleanText(raw?.source_title,180):'', source_url:verified?url:'' };
}
function sanitizeSignal(raw, sourceMap) {
  const url=cleanUrl(raw?.source_url), verified=Boolean(url&&sourceMap.has(url));
  return { score:verified&&Number.isFinite(Number(raw?.score))?Math.max(0,Math.min(100,Number(raw.score))):null, basis:cleanText(raw?.basis,260), source_url:verified?url:'' };
}
function sanitizePremium(raw, sourceMap) {
  const url=cleanUrl(raw?.source_url), verified=Boolean(url&&sourceMap.has(url));
  return { value:verified&&Number.isFinite(Number(raw?.value))?Number(raw.value):null, basis:cleanText(raw?.basis,260), source_url:verified?url:'' };
}

function sanitizeWindowResearch(parsed, profile, requested) {
  const {sources,sourceMap}=sanitizeSources(parsed.sources), requestMap=new Map(requested.map(window=>[window.id,window])), windows=[];
  for(const raw of Array.isArray(parsed.windows)?parsed.windows:[]){
    const expected=requestMap.get(cleanText(raw.id,100));if(!expected||raw.start!==expected.start||raw.end!==expected.end)continue;
    windows.push({ id:expected.id,start:expected.start,end:expected.end,summary:cleanText(raw.summary,420),flight_observed:sanitizeObservation(raw.flight_observed,sourceMap),lodging_observed:sanitizeObservation(raw.lodging_observed,sourceMap),weather:sanitizeSignal(raw.weather,sourceMap),saturation:sanitizeSignal(raw.saturation,sourceMap),logistics:sanitizeSignal(raw.logistics,sourceMap),event_premium:sanitizePremium(raw.event_premium,sourceMap),cautions:(Array.isArray(raw.cautions)?raw.cautions:[]).map(value=>cleanText(value,220)).filter(Boolean).slice(0,6) });
  }
  return { contract:WINDOW_CONTRACT_VERSION,parent_contract:CONTRACT_VERSION,destination:cleanText(parsed.destination||profile.destination_scope.values.join(', '),160),verified_at:new Date().toISOString(),summary:cleanText(parsed.summary,700),windows,sources,cautions:(Array.isArray(parsed.cautions)?parsed.cautions:[]).map(value=>cleanText(value,260)).filter(Boolean).slice(0,12) };
}

async function researchWindows(profile, windows, env) {
  const {payload,requestId}=await openAI({
    model:env.OPENAI_RESEARCH_MODEL||env.OPENAI_MODEL||'gpt-5',tools:[{type:'web_search',search_context_size:'high'}],max_output_tokens:7000,
    input:[{role:'system',content:'You are the comparative date-window research layer for Viajes ASC. Use current web search and strict source traceability. Never fabricate prices, forecasts, demand, availability, percentages or URLs.'},{role:'user',content:windowResearchPrompt(profile,windows)}],
    text:{format:{type:'json_schema',name:'asc_travel_window_research',strict:true,schema:windowResearchSchema}}
  },env,48000);
  const parsed=JSON.parse(responseText(payload));
  return { window_research:sanitizeWindowResearch(parsed,profile,windows),request_id:requestId,mode:'openai_window_web_search' };
}

export default {
  async fetch(request, env) {
    const started = Date.now(), requestId = crypto.randomUUID(), origin = request.headers.get('origin') || '', headers = cors(origin, env), url = new URL(request.url);
    if (request.method === 'GET' && url.pathname === '/health') return json({ status:'ok',service:SERVICE,contractVersion:CONTRACT_VERSION,windowContractVersion:WINDOW_CONTRACT_VERSION,research:'web_search',phase5:'inverse_windows',model:env.OPENAI_RESEARCH_MODEL||env.OPENAI_MODEL||'gpt-5' },200,headers);
    if (request.method === 'OPTIONS') { if (origin && !allowedOrigins(env).has(origin)) return json({ error: 'origin_not_allowed' }, 403, headers); return new Response(null,{status:204,headers}); }
    if (request.method !== 'POST') return json({ error: 'method_not_allowed' }, 405, headers);
    if (origin && !allowedOrigins(env).has(origin)) return json({ error: 'origin_not_allowed' }, 403, headers);
    const session=cleanText(request.headers.get('x-asc-session'),80), rate=await env.ASSISTANT_RATE_LIMITER.limit({key:session||'anonymous-travel-assistant'});
    ...
    try{
      const body=await readJsonBounded(request);if(!ALLOWED_ACTIONS.has(body?.action))return json({error:'invalid_action'},400,headers);
      const profile=safeProfile(body.profile);let result;
      if(body.action==='research_trip')result=await research(profile,env);
      else if(body.action==='research_windows')result=await researchWindows(profile,safeWindows(body.windows),env);
      else result=await summarize(profile,env);
      console.log(JSON.stringify({event:body.action,request_id:requestId,status:'ok',latency_ms:Date.now()-started,items:result.research?.items?.length||0,windows:result.window_research?.windows?.length||0}));
      return json({...result,gateway_request_id:requestId},200,headers);
    }catch(error){
      const code=cleanText(error?.message,240)||'internal_error';
      const status=code==='payload_too_large'?413:['invalid_profile','invalid_action','invalid_windows','missing_body'].includes(code)||error instanceof SyntaxError?400:error?.name==='TimeoutError'?504:502;
