...
def get(u,attempts=3,read_timeout=30):
 ...
 for i in range(attempts):
  try:
   r=requests.get(u,headers={'User-Agent':UA,'Accept':'text/csv,application/vnd.ms-excel,text/html;q=0.8,*/*;q=0.5'},timeout=(10,read_timeout)); r.raise_for_status(); return r.content,r.url
...
def main():
 ...
 fred_raw,_=get(FRED,3,30); rates=pd.read_csv(io.BytesIO(fred_raw)); dcol=next(c for c in rates.columns if 'date' in c.lower()); vcol=next(c for c in rates.columns if c!=dcol); rates[vcol]=pd.to_numeric(rates[vcol],errors='coerce'); rates[dcol]=pd.to_datetime(rates[dcol],errors='coerce'); rates=rates.dropna(subset=[vcol,dcol]); ten=float(rates.iloc[-1][vcol]); rates['ym']=rates[dcol].dt.strftime('%Y-%m'); monthly=rates[['ym',vcol]].copy()
