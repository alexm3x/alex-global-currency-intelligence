...
async function inspectPage(url) {
  try {
    const response = await timedFetch(url, { redirect: 'manual', headers: { 'user-agent': 'ViajesASC-Phase13-Verification/1.0' } });
    ...
    return { ok: response.status === 200 && bodyMarker, status: response.status, location, bodyMarker, error: null };
  } catch (error) {
    return { ok: false, status: 0, location: null, bodyMarker: false, error: String(error?.message || error) };
  }
}
...
async function inspectWorker() {
  ...
  try {
    const response = await timedFetch(`${workerUrl}/health`, { headers: { 'user-agent': 'ViajesASC-Phase13-Verification/1.0' } });
...
if (!pages.ok) externalBlockers.push(pages.location ? 'github_pages_route_redirected_or_unreachable' : 'github_pages_route_unreachable');
